...
import { EyeIcon, ImageIcon, ImageUpIcon, LockIcon, PanelTopIcon } from "lucide-react";
...
export default function CreateProjectDialog({ open, onClose }: CreateProjectDialogProps) {
  ...
  const canSubmit = nameValid && descriptionValid && publicSlug.trim().length > 0 && !loading;
      ...
      <DialogContent>
        ...
        <div className="relative">
          ...
          <div className="absolute z-[2] bottom-[-30px] left-[50%] translate-x-[-50%]">
            <div className="relative">
              {/* <span className="absolute z-[2] top-2 h-fit text-xs left-2 border px-2 py-1 mr-1 bg-background rounded-xs">Upload</span> */}
              <ImageUpload icon={<ImageUpIcon />} text="Project logo" className=" w-[120px] h-[120px] bg-popover" value={logo} onChange={setLogo} />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-1 mt-8">
          ...
          <Input id="name" placeholder="Project name" value={name} onChange={(e) => setName(e.target.value)} maxLength={20} />
